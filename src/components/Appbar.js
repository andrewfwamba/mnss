...
function Appbar() {
  ...
  return (
    <div>
      <nav className="py-3 md:py-5 fixed top-0 w-full flex px-4 md:px-32 justify-between items-center shadow-lg bg-white/90 z-10">
        ...
        <div className="top-[4rem] fixed uppercase w-full bg-green-600">
          <ul className="flex px-10 flex-col mt-5 w-full text-gray-200 font-[500]">
            {Menus.map((menu, index) => (
              <li
                key={index}
                className="flex space-x-3 justify-between items-center cursor-pointer hover:border-b-2 pb-4 border-blue-700/50"
              >
                <a href={`#${menu.id}`}>
                  <p>{menu.title}</p>
                </a>
